# Windows 11 Upgrade (25H2) – Technicians' Guide (v2.7.1)
...
- Toast assets/scripts live in `WindowsUpdate\Toast-Notification`; tasks schedule reboot reminders. Post-reboot validation now uses a SYSTEM scheduled task (`Win11_PostRebootValidation`, ONSTART) triggered by RunOnce to relaunch the orchestrator and perform cleanup.
- ISO download uses BITS with 5-minute inactivity detection and Invoke-WebRequest fallback, with health/hash validation and retries before staging.
- Self-repair restages from ISO after interrupted runs or a rebooted `PendingReboot` state, re-registering reminders/validation and reissuing the reboot toast.
...
- Post-reboot validation registers `Win11_PostRebootValidation` (SYSTEM, ONSTART) and a RunOnce to trigger it; the task replays `Windows11Upgrade.ps1` to validate cleanup.
...
- Hardware checks: TPM 2.0, Secure Boot, 64-bit CPU, RAM ≥ 4 GB, disk space check (64 GB default; unknown free space blocks staging).
...
- RunOnce plus a SYSTEM scheduled task handle post-reboot validation; ConnectWise RMM can still trigger `PostUpgradeCleanup.ps1` after the device has upgraded.
...
## 2.7.1 - 2025-12-04
- Post-reboot validation now registers a SYSTEM ONSTART task plus RunOnce trigger to avoid user-context cleanup failures.
- ISO download hardened with 5-minute inactivity detection, Invoke-WebRequest fallback, and aggressive removal of corrupt media/hash cache before retries.
- setup.exe monitoring resets stall timers on progress or CPU activity and logs exit codes with cataloged remediation; recoverable codes attempt self-repair before failing.
- Self-repair/staging paths re-register reminders/validation consistently and preserve clear failure markers on fatal paths.
